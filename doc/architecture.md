...
The event runtime separates generic sources from per-request conditions. Runtime starts and stops one background `EventSupervisor`, which is the sole owner of source polling for worker waits and autonomous subscribers. The native `pivot_wait_event` tool is one-shot and remains scoped to workers, while an optional declarative Event-to-Stimulus bridge converts configured rising-edge conditions into durable observation stimuli. Bridge edge state and occurrences are persisted; occurrence, stimulus, and fired edge state are written atomically. Device protocols and concrete thresholds remain instance concerns. See [events](events.md) for the source, wait, and bridge contracts.
