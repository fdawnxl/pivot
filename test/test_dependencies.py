...
def _write_project(root: Path, dependency_id: str = "com.pivot.sensor-server") -> Path:
...
def test_dependency_manifest_requires_uv_project_and_valid_meaningful_id(tmp_path: Path) -> None:
    ...
    assert descriptor.dependency_id == "com.pivot.sensor-server"
...
def test_dependency_discovery_is_first_level_and_skips_invalid_or_duplicate_projects(
    ...
    first = _write_project(root / "first", "com.pivot.shared")
    _write_project(root / "duplicate", "com.pivot.shared")
    valid = _write_project(root / "valid", "com.pivot.valid")
    invalid = root / "invalid"
    invalid.mkdir(parents=True)
    (invalid / "dependency.toml").write_text('id = "com.pivot.invalid"\ncommand = "python server.py"\n', encoding="utf-8")
    _write_project(first / "nested", "com.pivot.nested")
...
class FakeStatusClient:
    ...
    def query(self, dependency_id: str, *, timeout: float) -> DependencyStatus:
        self.queries.append((dependency_id, timeout))
        return DependencyStatus(dependency_id, "ready", "available", {"samples": 1})

    def ping(self, dependency_id: str, *, timeout: float) -> bool:
        self.pings.append((dependency_id, timeout))
        return True
...
def test_manager_syncs_only_once_runs_without_sync_and_uses_dbus_status(tmp_path: Path) -> None:
    ...
    assert first == (DependencyStatus("com.pivot.sensor-server", "ready", "available", {"samples": 1}),)
    assert sync_calls[0][0] == ["uv", "sync", "--project", str(project.resolve())]
    assert (project / DEPENDENCY_READY_MARKER).read_text(encoding="utf-8") == "com.pivot.sensor-server\n"
    ...
    assert launch_calls[0][1]["env"]["PIVOT_DEPENDENCY_ID"] == "com.pivot.sensor-server"
    assert status_client.pings == [("com.pivot.sensor-server", 0.25)]
    assert status_client.queries == [("com.pivot.sensor-server", 0.25)]

    assert manager.stop("com.pivot.sensor-server")
    assert processes[0].terminated
    manager.start("com.pivot.sensor-server")
...
def test_start_all_isolates_one_dependency_installation_failure(tmp_path: Path) -> None:
    ...
    assert [status.dependency_id for status in statuses] == ["com.pivot.healthy"]
    assert launched == ["com.pivot.healthy"]
    manager.close()
...
def test_status_payload_rejects_mismatched_id_and_invalid_state() -> None:
    with pytest.raises(DependencyError, match="different id"):
        DependencyStatus.from_payload("com.pivot.sensor", {"id": "com.pivot.other", "state": "ready"})
    with pytest.raises(DependencyError, match="invalid state"):
        DependencyStatus.from_payload("com.pivot.sensor", {"id": "com.pivot.sensor", "state": "unknown"})
...
def test_common_dbus_client_queries_status_and_heartbeat(tmp_path: Path) -> None:
    ...
    service.write_text(
        ...
        " def __init__(self): super().__init__('com.pivot.Dependency1')\n"
        " @method()\n"
        " def Ping(self)->'s': return 'com.pivot.integration'\n"
        " @method()\n"
        " def GetStatus(self)->'s': return json.dumps({'id':'com.pivot.integration','state':'ready','message':'ok'})\n"
        "async def main():\n"
        " bus=await MessageBus(bus_address='" + address + "').connect()\n"
        " bus.export('/com/pivot/Dependency',Service())\n"
        " await bus.request_name('com.pivot.integration')\n"
        " await asyncio.Future()\n"
        "asyncio.run(main())\n",
        encoding="utf-8",
    )
    service_process = subprocess.Popen([sys.executable, str(service)])
    client = DBusDependencyStatusClient(bus_address=address)
    try:
        deadline = time.monotonic() + 3
        while time.monotonic() < deadline and not client.ping("com.pivot.integration", timeout=0.5):
            time.sleep(0.05)
        assert client.ping("com.pivot.integration", timeout=0.5)
        assert client.query("com.pivot.integration", timeout=0.5) == DependencyStatus(
            "com.pivot.integration", "ready", "ok"
        )
    finally:
