...
def test_logging_uses_independent_console_and_file_levels(tmp_path: Path) -> None:
    ...
    assert [item["message"] for item in persisted[-3:]] == [
        "debug detail",
        "normal operation",
        "failure detail",
    ]
...
def test_structured_logging_includes_correlation_context(tmp_path: Path) -> None:
    ...
    with log_context(
        correlation_id="request-1", activation_id="activation-1", agent_id="agent-1"
    ):
        logging.getLogger("pivot.test").info(
            "correlated operation", extra={"capability": "cpu_read"}
        )
    for handler in logging.getLogger().handlers:
        handler.flush()
...
def test_logging_level_environment_precedence(
    ...
    CredentialStore(instance / "credentials.toml").save(
        {"test": ProviderCredential("test", "model")}
    )
