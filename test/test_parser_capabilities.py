...
def test_instance_capability_discovery_loads_all_kinds(tmp_path: Path) -> None:
    instance = tmp_path
    for kind in ("think", "measure", "work"):
        (instance / "capabilities" / kind).mkdir(parents=True)
        environment = instance / "environment" / kind
